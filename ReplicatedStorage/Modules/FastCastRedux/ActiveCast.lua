...
-- If penetrate callback has to run more than this many times, it will register a hit and stop calculating penetrations.
-- This only applies for repeated penetrations, e.g. the amount of parts that fit within the space of a single cast segment (NOT the whole bullet's trajectory over its entire lifetime)
...
local function SendRayFinalHit(cast: ActiveCast, origin: Vector3, direction: Vector3, resultOfCast: RaycastResult, segmentVelocity: Vector3, cosmeticBulletObject: Instance?, blocked: boolean?)
	--cast.RayFinalHit:Fire(cast, resultOfCast, segmentVelocity, cosmeticBulletObject)
	cast.Caster.RayFinalHit:Fire(cast, origin, direction, resultOfCast, segmentVelocity, cosmeticBulletObject, blocked)
end

local function SendRayHit(cast: ActiveCast, origin: Vector3, direction: Vector3, resultOfCast: RaycastResult, segmentVelocity: Vector3, cosmeticBulletObject: Instance?, reflected: boolean?)
	--cast.RayHit:Fire(cast, resultOfCast, segmentVelocity, cosmeticBulletObject)
	cast.Caster.RayHit:Fire(cast, origin, direction, resultOfCast, segmentVelocity, cosmeticBulletObject, reflected)
end
...
local function SimulateCast(cast: ActiveCast, delta: number, expectingShortCall: boolean)
	...
	if cast.RayInfo.RaycastHitbox then
		...
		if cast.RayInfo.ShapeCast then
			resultOfCast = cast.RayInfo.ShapeCast.CastShape == "Block" and targetWorldRoot:Blockcast(cast.RayInfo.CurrentCFrame, cast.RayInfo.ShapeCast.CastSize, rayDir, cast.RayInfo.Parameters) or targetWorldRoot:Spherecast(lastPoint, cast.RayInfo.ShapeCast.CastRadius, rayDir, cast.RayInfo.Parameters) --cast.RayInfo.CurrentCFrame.Position
		else
	...
	local rayDisplacement = (((cast.RayInfo.RaycastHitbox or cast.RayInfo.ShapeCast) and currentTarget or point) - lastPoint).Magnitude
	...
	SendLengthChanged(cast, ((cast.RayInfo.RaycastHitbox or cast.RayInfo.ShapeCast) and currentTarget or point), lastPoint, rayDir.Unit, rayDisplacement, segmentVelocity, cast.RayInfo.CosmeticBulletObject)
		...
		if cast.RayInfo.CanPenetrateCallback == nil or (cast.RayInfo.CanPenetrateCallback ~= nil and cast.RayInfo.CanPenetrateCallback(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject) == false) then
			...
			if (cast.StateInfo.HighFidelityBehavior == 2 and latestTrajectory.Acceleration ~= Vector3.new() and cast.StateInfo.HighFidelitySegmentSize ~= 0) then
				...
				for segmentIndex = 1, numSegmentsReal do
					...
					if cast.RayInfo.RaycastHitbox then
						...
						if cast.RayInfo.ShapeCast then
							subResult = cast.RayInfo.ShapeCast.CastShape == "Block" and targetWorldRoot:Blockcast(cast.RayInfo.CurrentCFrame, cast.RayInfo.ShapeCast.CastSize, subRayDir, cast.RayInfo.Parameters) or targetWorldRoot:Spherecast(subPosition, cast.RayInfo.ShapeCast.CastRadius, subRayDir, cast.RayInfo.Parameters) --cast.RayInfo.CurrentCFrame.Position
						else
					...
					if (subResult ~= nil) then
						...
						if cast.RayInfo.CanPenetrateCallback == nil or (cast.RayInfo.CanPenetrateCallback ~= nil and cast.RayInfo.CanPenetrateCallback(cast, origin, subRayDir.Unit, subResult, subVelocity, cast.RayInfo.CosmeticBulletObject) == false) then
							if cast.RayInfo.CanHitCallback == nil then
								...
								local canHit = cast.RayInfo.CanHitCallback(cast, origin, subRayDir.Unit, subResult, subVelocity, cast.RayInfo.CosmeticBulletObject)
								if canHit == false or canHit == "Blocked" then
									-- Still hit even at high res
									cast.StateInfo.IsActivelyResimulating = false

									SendRayFinalHit(cast, origin, subRayDir.Unit, subResult, subVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Blocked")
									cast:Terminate(subResult)
									local vis = DbgVisualizeHit(CFrame.new(point), false)
									if (vis ~= nil) then vis.Color3 = Color3.new(0.0588235, 0.87451, 1) end
									return
									...
									SendRayHit(cast, origin, subRayDir.Unit, subResult, subVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Reflected") -- This may result in CancelHighResCast being set to true.
			...
			elseif (cast.StateInfo.HighFidelityBehavior ~= 1 and cast.StateInfo.HighFidelityBehavior ~= 3) then
				...
				if cast.RayInfo.CanHitCallback == nil then
					...
					local canHit = cast.RayInfo.CanHitCallback(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject)
					if canHit == false or canHit == "Blocked" then
						-- This is not a physics cast, or recalculation is off.
						PrintDebug("Hit was successful. Terminating.")
						SendRayFinalHit(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Blocked")
						cast:Terminate(resultOfCast)
						DbgVisualizeHit(CFrame.new(point), false)
						return
					else
						-- Recalculating hit something hittable instead.
						SendRayHit(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Reflected") -- This may result in CancelHighResCast being set to true.
						local vis = DbgVisualizeHit(CFrame.new(point), true)
						if (vis ~= nil) then vis.Color3 = Color3.new(1, 0.113725, 0.588235) end
			...
			local function Recursive()
				...
				while true do
					...
					if cast.RayInfo.ShapeCast then
						resultOfCast = cast.RayInfo.ShapeCast.CastShape == "Block" and targetWorldRoot:Blockcast(cast.RayInfo.CurrentCFrame, cast.RayInfo.ShapeCast.CastSize, rayDir, params) or targetWorldRoot:Spherecast(lastPoint, cast.RayInfo.ShapeCast.CastRadius, rayDir, params) --cast.RayInfo.CurrentCFrame.Position
					else
				...
				if brokeFromSolidObject then
					if cast.RayInfo.CanHitCallback == nil then
						...
						local canHit = cast.RayInfo.CanHitCallback ~= nil and cast.RayInfo.CanHitCallback(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject)
						if canHit == false or canHit == "Blocked" then
							reallyBrokeFromSolidObject = true
							blocked = (canHit == "Blocked")
						else
							-- Recalculating hit something hittable instead.
							SendRayHit(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Reflected") -- This may result in CancelHighResCast being set to true.
							local vis = DbgVisualizeHit(CFrame.new(point), true)
			...
			if reallyBrokeFromSolidObject then
				...
				SendRayFinalHit(cast, origin, rayDir.Unit, resultOfCast, segmentVelocity, cast.RayInfo.CosmeticBulletObject, blocked)
				cast:Terminate(resultOfCast)
	...
	if part and part ~= cast.RayInfo.CosmeticBulletObject then
		if cast.RayInfo.CanPenetrateCallback == nil or (cast.RayInfo.CanPenetrateCallback ~= nil and cast.RayInfo.CanPenetrateCallback(cast, origin, direction, resultOfCast, initialVelocity, cast.RayInfo.CosmeticBulletObject) == false) then
			...
			if cast.RayInfo.CanHitCallback == nil then
				...
				local canHit = cast.RayInfo.CanHitCallback(cast, origin, direction, resultOfCast, initialVelocity, cast.RayInfo.CosmeticBulletObject)
				if canHit == false or canHit == "Blocked" then
					PrintDebug("Hit was successful. Terminating.")
					SendRayFinalHit(cast, origin, direction, resultOfCast, initialVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Blocked")
					DbgVisualizeHit(CFrame.new(point), false)
				else
					SendRayHit(cast, origin, direction, resultOfCast, initialVelocity, cast.RayInfo.CosmeticBulletObject, canHit == "Reflected") -- This may result in CancelHighResCast being set to true.
					local vis = DbgVisualizeHit(CFrame.new(point), true)
					if (vis ~= nil) then vis.Color3 = Color3.new(1, 0.113725, 0.588235) end
...
function ActiveCastStatic.new(caster: Caster, origin: Vector3, direction: Vector3, velocity: Vector3 | number, castDataPacket: FastCastBehavior): ActiveCast
	...
	local cast = {
		...
		StateInfo = {
			...
			IsActivelySimulatingPenetration = false,
			...
			Trajectories = {
				{
					...
					Acceleration = castDataPacket.Hitscan and Vector3.new(0, 0, 0) or castDataPacket.Acceleration
				}
			},
			TweenTable = {},
		},
		...
		RayInfo = {
			...
			WorldRoot = workspace,
			TravelType = castDataPacket.TravelType or "Distance",
			MaxDistance = castDataPacket.MaxDistance or 1000,
			Lifetime = castDataPacket.Lifetime or 5,
			CosmeticBulletObject = castDataPacket.CosmeticBulletTemplate, -- This is intended. We clone it a smidge of the way down.
			CanPenetrateCallback = castDataPacket.CanPenetrateFunction,
			CanHitCallback = castDataPacket.CanHitFunction,
			RaycastHitbox = castDataPacket.RaycastHitbox,
			ShapeCast = castDataPacket.ShapeCast,
			CurrentCFrame = castDataPacket.CurrentCFrame,
			ModifiedDirection = castDataPacket.ModifiedDirection, 
		},
		
		UserData = castDataPacket.UserData or {}
	}
	...
	if castDataPacket.CosmeticBulletProvider == nil then
		...
		if cast.RayInfo.CosmeticBulletObject ~= nil then
			...
			for _, v in pairs(cast.RayInfo.CosmeticBulletObject:GetDescendants()) do
				if v:IsA("Sound") then
					v:Play()
				end
			end	
		...
		if typeof(castDataPacket.CosmeticBulletProvider) == "PartCache" then
			...
			local Initalized = cast.RayInfo.CosmeticBulletObject:FindFirstChild("Initalized")
			if Initalized then				
				local CoreTransparency = cast.RayInfo.CosmeticBulletObject:FindFirstChild("CoreTransparency")
				if CoreTransparency then
					cast.RayInfo.CosmeticBulletObject.Transparency = CoreTransparency.Value	
				end
				for _, v in pairs(cast.RayInfo.CosmeticBulletObject:GetDescendants()) do
					if v:IsA("ParticleEmitter") or v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") or v:IsA("Trail") or v:IsA("Beam") or v:IsA("BillboardGui") or v:IsA("SurfaceGui") then
						v.Enabled = true
					...
					elseif v:IsA("Decal") or v:IsA("Texture") or v:IsA("BasePart") then
						local CurrentTransparency = v:FindFirstChild("CurrentTransparency")
						if CurrentTransparency then
							v.Transparency = CurrentTransparency.Value					
						end
					end
				end
			else
				local InitalizedClone = Instance.new("BoolValue")
				InitalizedClone.Name = "Initalized"
				InitalizedClone.Parent = cast.RayInfo.CosmeticBulletObject
				local CoreTransparency = Instance.new("NumberValue")
				CoreTransparency.Name = "CoreTransparency"
				CoreTransparency.Value = cast.RayInfo.CosmeticBulletObject.Transparency
				CoreTransparency.Parent = cast.RayInfo.CosmeticBulletObject
				for _, v in pairs(cast.RayInfo.CosmeticBulletObject:GetDescendants()) do
					if v:IsA("ParticleEmitter") or v:IsA("PointLight") or v:IsA("SurfaceLight") or v:IsA("SpotLight") or v:IsA("Trail") or v:IsA("Beam") or v:IsA("BillboardGui") or v:IsA("SurfaceGui") then
						v.Enabled = true
					elseif v:IsA("Sound") then
						v:Play()
					elseif v:IsA("Decal") or v:IsA("Texture") or v:IsA("BasePart") then
						local CurrentTransparency = Instance.new("NumberValue")
						CurrentTransparency.Name = "CurrentTransparency"
						CurrentTransparency.Value = v.Transparency
						CurrentTransparency.Parent = v
					end
				end				
			end
	...
	if castDataPacket.Hitscan then
		local InitialVelocity = typeof(velocity) == "number" and (direction.Unit * velocity) or velocity
		local Distance = cast.RayInfo.TravelType == "Lifetime" and (InitialVelocity.Magnitude * cast.RayInfo.Lifetime) or cast.RayInfo.MaxDistance
		SimulateHitscan(cast, origin, direction.Unit, InitialVelocity, Distance)
		task.spawn(function()
			for i, v in pairs(cast.StateInfo.TweenTable) do
				cast.StateInfo.UpdateConnection = event:Connect(function(delta)
					if not v.ready then
						cast.StateInfo.Delta = delta
						cast.StateInfo.TotalRuntime += delta
						local new = InitialVelocity.Magnitude * v.s
						if v.s >= 0 then
							local fd = math.min(new, v.l)							
							local currentPoint = v.direction * Vector3.new(0, 0, -fd)
							local currentVelocity = v.direction.LookVector * InitialVelocity.Magnitude							
							local point = currentPoint
							local rayDisplacement = (point - v.lastPoint).Magnitude
							SendLengthChanged(cast, v.lastPoint, point, v.direction.LookVector, rayDisplacement, currentVelocity, cast.RayInfo.CosmeticBulletObject)
							local rayVisualization: ConeHandleAdornment? = nil
							if (delta > 0) then
								rayVisualization = DbgVisualizeSegment(CFrame.new(v.lastPoint, currentPoint), rayDisplacement)								
							end
							v.lastPoint = currentPoint
							v.ready = fd >= v.l
						end
						v.s = v.s + delta							
					end
				end)
				repeat event:Wait() until v.ready
